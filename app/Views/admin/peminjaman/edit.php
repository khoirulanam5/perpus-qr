...
<div class="card">
    <div class="card-body">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                <h5 class="fw-semibold text-dark text-uppercase mb-0">Edit Peminjaman</h5>
            </div>
            <div>
        ...
        <form action="<?= base_url('peminjaman/' . $peminjaman->id) ?>" method="POST" enctype="multipart/form-data">
            <?= csrf_field() ?>
                ...
                <div class="col-sm-10">
                    <select id="anggota_id" name="anggota_id"
                        ...
                        <?php foreach ($anggotas as $anggota) : ?>
                            <option value="<?= $anggota->id; ?>" <?= old('anggota_id', $peminjaman->anggota_id) == $anggota->id ? 'selected' : ''; ?>><?= $anggota->kode_anggota . ' | ' . $anggota->nama; ?></option>
                        <?php endforeach; ?>
                    </select>
            ...
            <div class="form-group row mb-3">
                <label class="control-label col-sm-2 align-self-center mb-0 text-uppercase text-dark"
                    style="font-weight: 600; font-size: 14px" for="tanggal_kembali">Tanggal Kembali </label>
                <div class="col-sm-10">
                    <input type="date" name="tanggal_kembali" id="tanggal_kembali"
                        class="form-control text-dark <?= session('errors.tanggal_kembali') ? 'is-invalid' : '' ?>"
                    ...
                    <?php if (session('errors.tanggal_kembali')) : ?>
                        <div class="invalid-feedback"><?= session('errors.tanggal_kembali') ?></div>
                    <?php endif ?>
                </div>
            </div>
                            ...
                            <tr class="text-dark">
                                <td class="text-center checkbox-col">
                                    <input type="checkbox" name="buku[]" value="<?= $data->id ?>" class="form-check-input border-secondary"
                                        <?= in_array($data->id, $bukuDipinjam) ? 'checked' : '' ?>>
                                </td>
...
<script>
    ...
    $('#dataTable').DataTable({
        ...
        order: [
            [0, 'desc']
        ], // ⬅️ Urutkan berdasarkan kolom ke-1 (index 1 = kolom "No")
        columnDefs: [{
            targets: 0,
            orderDataType: 'dom-checkbox' // Kolom checkbox
        }],
