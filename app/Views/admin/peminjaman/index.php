                ...
                <tbody>
                    ...
                    foreach ($peminjamans as $data): ?>
                        <tr class="text-dark">
                            ...
                            <td><?= $data->tanggal_due ?></td>
                            <td>
                                <?= $data->tanggal_kembali ? formatTanggalIndo($data->tanggal_kembali) : '<span class="badge bg-warning">Belum</span>' ?>
                            </td>
                            <td>
                                <?php
                                $selisihHari = hitungSelisihHari($data->tanggal_due) - $pengaturanDenda->maksimal_telat;
                                ...
                                if ($data->total_denda == 0) {
                                    ...
                                    echo $denda > 0
                                        ? "<span class='badge bg-danger text-white'>
                                            <div>" . number_format($denda, 0, ',', '.') . "</div>
                                        </span>
                                        <br>"
                                        : '';
                                ?>
                                    <?php
                                    if (!empty($data->tanggal_kembali) && $data->total_denda > 0) {
                                        $selisihHari = hitungSelisihHariDenda($data->tanggal_due, $data->tanggal_kembali) - $pengaturanDenda->maksimal_telat;
                                    } else {
                                        $selisihHari = hitungSelisihHari($data->tanggal_due) - $pengaturanDenda->maksimal_telat;
                                    }
                                    ?>
                                ...
                                <td>
                                    <div class="d-block">
                                        <div class="d-flex gap-1">
                                            <a href="<?= base_url('peminjaman/edit/' . $data->id) ?>" class="btn btn-info btn-sm">
                                                <i class="bx bx-pencil" style="font-size: 1rem"></i>
                                            </a>
                                            <form action="<?= base_url('peminjaman/' . $data->id) ?>" method="POST" onsubmit="return confirm('Yakin ingin menghapus data ini?');">
                                                <?= csrf_field() ?>
                                        ...
                                        <?php
                                        $selisihHari = hitungSelisihHari($data->tanggal_due) - $pengaturanDenda->maksimal_telat;
                                        if (!$data->total_denda && $selisihHari > 0): ?>
                                            <a href="<?= base_url('denda/' . $data->id . '/create') ?>" class="btn btn-danger btn-sm mt-1" style="width: 5.3rem">
                                                Denda
                                            </a>
                                        <?php endif; ?>
                                    </div>
