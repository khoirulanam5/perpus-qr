...
<div class="card shadow">
    <div class="card-body">
        <div class="d-flex justify-content-between align-items-center mb-2">
            <p class="fw-semibold text-dark fs-4 col-6">Data Tema</p>
            <?php if (session()->get('user_role') == 'admin') { ?>
                <a href="<?= base_url('tema/create') ?>" class="btn btn-primary fw-semibold col-lg-2 col-4">Tambah</a>
            <?php } ?>
        </div>
                        ...
                        <tr class="text-dark">
                            ...
                            <?php if (session()->get('user_role') == 'admin') { ?>
                                ...
                                <td class="d-flex gap-1">
                                    <a href="<?= base_url('tema/edit/' . $data->id) ?>" class="btn btn-info btn-sm">
                                        <i class="bx bx-pencil" style="font-size: 1rem"></i>
                                    </a>
                                    <form action="<?= base_url('tema/' . $data->id) ?>" method="POST" onclick="return confirm('Apakah Anda yakin ingin menghapus data ini?');">
                                        <?= csrf_field() ?>
